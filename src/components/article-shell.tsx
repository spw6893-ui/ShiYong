...
export function ArticleShell({title, summary, eyebrow, readingTime, publishedAt, verifiedAt, reviewOverdue = false, content, backHref = "/resources", backLabel = "返回资料库"}: ArticleShellProps) {
  return <article className="article-shell">
    ...
    <div className="article-layout"><div className="article-content"><MDXContent code={content} components={mdxComponents} /></div><aside className="article-side"><div><b>内容原则</b><p>方法优先，工具其次。</p><p>重要信息需要核查。</p><p>敏感资料不要上传。</p></div><a href="/resources">继续浏览资料库 <Icon icon="solar:arrow-right-linear" /></a></aside></div>
  </article>;
}
