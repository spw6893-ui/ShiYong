...
const scenarios = ["全部任务", "处理 PDF", "制作 PPT", "处理表格", "整理会议", "搜索资料", "写作改稿", "处理图片", "绘制图表", "学习资料", "综合任务"];

export function LibraryBrowser({items}: {items: LibraryCardData[]}) {
  const [query, setQuery] = useState("");
  const [type, setType] = useState("全部");
  const [scenario, setScenario] = useState("全部任务");
  ...
  useEffect(() => {
    ...
    setScenario(params.get("scenario") ?? "全部任务");
  }, []);

  const filtered = useMemo(() => items.filter((item) => {
    const matchQuery = `${item.title}${item.summary}${item.type}${item.scenario}`.toLowerCase().includes(query.toLowerCase());
    return matchQuery && (type === "全部" || item.type === type) && (scenario === "全部任务" || item.scenario === scenario) && (cost === "全部成本" || item.cost === cost);
  }), [items, query, type, scenario, cost]);

  const reset = () => { setQuery(""); setType("全部"); setScenario("全部任务"); setCost("全部成本"); };

  return <div className="inner-shell">
    <section className="resource-page-hero"><span>AI TASK LIBRARY</span><h1>你手上是 PDF、PPT，还是一张表格？</h1><p>直接按要处理的文件和任务筛选，查看当前工具入口、具体步骤、提示模板和限制。</p><Input value={query} onValueChange={setQuery} placeholder="搜索：总结 PDF、重做 PPT、清洗 Excel、整理会议……" startContent={<Icon icon="solar:magnifer-linear" />} classNames={{inputWrapper: "resource-page-search"}} /></section>
    ...
    <section className="library-filter-panel" aria-label="资料筛选">
      ...
      <div className="filter-row"><b>任务</b><div className="filter-tabs">{scenarios.map((item) => <button className={scenario === item ? "active" : ""} onClick={() => setScenario(item)} key={item}>{item}</button>)}</div></div>
    ...
    <div className="library-result-bar"><span>找到 <b>{filtered.length}</b> 条资料</span>{(query || type !== "全部" || scenario !== "全部任务" || cost !== "全部成本") && <button onClick={reset}>清除筛选</button>}</div>
